...
//base for all Events
class EventLocation {
    constructor(name, address, zip, img) {
        this.name = name;
...
class Restaurant extends EventLocation {
    constructor(name, address, zip, img, cuisine, phone, website) {
...
class Events extends EventLocation {
    constructor(name, address, zip, img, price, eventDate, eventTime) {
        super(name, address, zip, img);
        this.price = price;
...
//event array
let eventArray = [charlesChurch, schoenbrunn, lemonLeaf, sixta, kristofferson, kravitz];
//printing in DOM
function printOnSite(array) {
    array.forEach((event) => {
        event.displayEvent();
    });
}
